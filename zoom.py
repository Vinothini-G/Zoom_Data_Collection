...
class Zoom:
    ...
    def login_zoom(self):
        ...
        account = WebDriverWait(self.browser, 10).until(EC.element_to_be_clickable((By.ID, "email")))
        account.send_keys(self.email)
        passwrd = WebDriverWait(self.browser, 10).until(EC.element_to_be_clickable((By.ID, "password")))
        passwrd.send_keys(self.password)
        # self.driver.get_screenshot_as_file("login1.png")

        # Click the login button
        element = self.browser.find_element_by_xpath("//div[@class='signin']")
    ...
    def launch_driver(self,duration=90): 
        ...
        #enter URL
        logger.info(f"joining meeting at {self.url}")
        self.browser.get(self.url)
        time.sleep(5) 
        # self.browser.get_screenshot_as_file("1-launch-driver.png")

        # click 'launch meeting'
        children = WebDriverWait(self.browser,20).until(EC.presence_of_all_elements_located((By.XPATH,'//div[text()="Launch Meeting"]')))
        ActionChains(self.browser).move_to_element(children[0]).click().perform()
        time.sleep(2)
        # self.browser.get_screenshot_as_file("2_launch_meeting.png")

        # click 'join from your browser'
        join = WebDriverWait(self.browser,30).until(EC.presence_of_all_elements_located((By.XPATH,'//a[text()="Join from Your Browser"]')))
        ActionChains(self.browser).move_to_element(join[0]).click().perform()
        time.sleep(2)
        # self.browser.get_screenshot_as_file("3_browser_join.png")

        # Enter guest name in input field
        name = WebDriverWait(self.browser, 20).until(EC.presence_of_all_elements_located((By.XPATH, '//input[@type="text"][@id="inputname"]')))
        name[0].send_keys('ucsb_guest')
        time.sleep(1)
        # self.browser.get_screenshot_as_file("4_enter_name.png")

        # Click 'Join' to join the meeting
        enterJoin = WebDriverWait(self.browser, 20).until(EC.presence_of_all_elements_located((By.XPATH, '//button[text()="Join"]')))
        ActionChains(self.browser).move_to_element(enterJoin[0]).click().perform()
        time.sleep(5)
        self.browser.get_screenshot_as_file("5_joined.png")
        logging.info("Successfully joined call")
